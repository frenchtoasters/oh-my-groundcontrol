...
const PROMPTS_DIR_NAME = 'oh-my-groundcontrol';
...
function loadConfigFromPath(configPath: string): PluginConfig | null {
  try {
    ...
    if (!result.success) {
      console.warn(`[oh-my-groundcontrol] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      console.warn(
        `[oh-my-groundcontrol] Error reading config from ${configPath}:`,
        error.message,
      );
    }
    return null;
 ...
 *
 * @param basePath - Base path without extension (e.g., /path/to/oh-my-groundcontrol)
 ...
 * 1. User config: ~/.config/opencode/oh-my-groundcontrol.jsonc or .json (or $XDG_CONFIG_HOME)
 * 2. Project config: <directory>/.opencode/oh-my-groundcontrol.jsonc or .json
 *
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[oh-my-groundcontrol] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
  ...
  const readFirstPrompt = (
    ...
    for (const dir of promptSearchDirs) {
      ...
      try {
        ...
        console.warn(
          `[oh-my-groundcontrol] ${errorPrefix} ${promptPath}:`,
